...
def GetStep(board, is_black):
    ...
    if(is_black):
        ...
        if len(step) == 0: #表示周圍沒得跳
            ismove = 0
            c = 0 #甩五次就不管了
            while ismove == 0:
                c += 1
                move = random.randint(0, len(blackpos)-1)  
                #隨機棋往右一個，如果右邊可以走，不能就繼續甩
                ...
                if c>=20:
                    if blackpos[move][1]+1<=7 and board[blackpos[move][0]][blackpos[move][1]+1] == 0:
                        step = [blackpos[move], (blackpos[move][0],blackpos[move][1]+1)] #隨機棋往右一個，如果右邊可以走，不能就繼續甩
                        ismove = 1
                    else:
                        continue
            ...
            while ismove == 0:
                ...
                if c>=20:
                    if whitepos[move][1]-1>=0 and board[whitepos[move][0]][whitepos[move][1]-1] == 0:
                        step = [whitepos[move], (whitepos[move][0],whitepos[move][1]-1)] #隨機棋往左一個
                        ismove = 1
                    else:
                        continue        
    else:
